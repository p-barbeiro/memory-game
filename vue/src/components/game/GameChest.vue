<template>
  <div class="grid gap-1 mt-5 content-center w-full md:w-auto" :style="style">
    <div v-for="(card, index) in cards" :key="index" class="md:w-40 relative aspect-square cursor-pointer select-none" @click="handleCardClick(card)">
      <!-- Card front -->
      <div class="absolute bg-gray-50 w-full h-full rounded flex justify-center items-center shadow transition-transform duration-500 transform preserve-3d p-5" :class="{ 'flip-up': card.flipped || card.matched, 'flip-down': !card.flipped && !card.matched, hidden: card.matched }">
        <img v-if="card.flipped || card.matched" :src="card.image" alt="Card" class="w-full h-full object-scale-down rounded scale-x-[-1]" />
      </div>
