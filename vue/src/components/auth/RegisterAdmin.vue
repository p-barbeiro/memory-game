...
const register = async () => {
  try {
    const response = await storeAuth.register(credentials.value, 'A')
    if (response) {
      toast({
        title: `Administrator created successfully!`,
        description: `Administrator ${credentials.value.name} has been created.`,
        variant: 'success'
      })
...
<template>
  <div class="place-self-center flex flex-col w-full md:flex-row justify-center items-center h-full">
