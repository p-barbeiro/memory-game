<template>
    <div class="w-full">
      ...
      <div>
        <div class="mt-5 flex flex-col md:flex-row w-full gap-3">
          ...
          <div v-if="apiData" class="border rounded-xl p-2 shadow md:w-1/3">
            <h2 class="text-lg font-semibold text-center mb-3">Single-Player Overview</h2>
            <Table>
      ...
      <div class="flex">
        <div class="flex flex-col md:flex-row w-full gap-3">
          ...
          <div v-if="multiPlayerOverview" class="border rounded-xl p-2 shadow md:w-1/2">
            <h2 class="text-lg font-semibold text-center">Multiplayer Overview</h2>
            <hr class="my-2" />
            <div>
              <Table>
                <TableBody>
      ...
      <h1 class="text-2xl font-semibold text-center text-indigo-900">Transactions Statistics</h1>
  
      <hr class="my-3" />
  
      <div class="flex flex-col md:flex-row w-full gap-3">
        <div v-if="transactionsOverview" class="border rounded-xl p-2 shadow md:w-2/3">
          <h2 class="text-lg font-semibold text-center">Transactions Overview</h2>
          <hr class="my-2" />
          <div>
            <Table>
              <TableBody>
                <TableRow class="flex p-1">
                  Total Transactions:
                  <span class="ml-auto">{{ transactionsOverview.totalTransactions }}</span>
                </TableRow>
                <TableRow class="flex p-1">
                  Total Spent:
                  <span class="ml-auto">{{ transactionsOverview.totalSpent }} €</span>
                </TableRow>
                <TableRow class="flex p-1">
                  Bonus Transactions:
                ...
                <TableRow class="flex p-1">
                  ...
                  <span class="ml-auto">{{ transactionsOverview.purchaseTransactions }}</span>
                </TableRow>
                <TableRow class="flex p-1">
                  Purchase Total Spent:
                  <span class="ml-auto">{{ transactionsOverview.purchaseTotalSpent }} €</span>
                </TableRow>
                <TableRow class="flex p-1">
        ...
        <div v-if="totalTransPieChart" class="border rounded-xl p-2 shadow md:w-1/3">
          <h2 class="text-lg font-semibold text-center">Total Transactions</h2>
          <hr class="my-2" />
          <div>
            <Pie :data="totalTransPieChart" />
          </div>
        </div>
      </div>
    </div>
  ...
  const fetchPersonalStatistics = async () => {
    ...
    try {
      ...
      const transactions = apiData.value.transactions
      transactionsOverview.value = {
        totalTransactions: transactions.totalPurchases,
        totalSpent: transactions.totalSpent,
  
        bonusTransactions: transactions.purchasesByType.B?.totalTransactions ?? 0,
        bonusBrainCoins: transactions.purchasesByType.B?.totalBrainCoins ?? 0,
        bonusAverageBrainCoins: transactions.purchasesByType.B?.averageBrainCoins ?? 0,
  
        inappTransactions: transactions.purchasesByType.I?.totalTransactions ?? 0,
        inappBrainCoins: transactions.purchasesByType.I?.totalBrainCoins ?? 0,
        inappAverageBrainCoins: transactions.purchasesByType.I?.averageBrainCoins ?? 0,
  
        purchaseTransactions: transactions.purchasesByType.P?.totalTransactions ?? 0,
        purchaseTotalSpent: transactions.purchasesByType.P?.totalSpent ?? 0,
        purchaseAverageSpent: transactions.purchasesByType.P?.averagePurchase ?? 0,
        purchaseTotalBrainCoins: transactions.purchasesByType.P?.totalBrainCoins ?? 0,
        purchaseAverageBrainCoins: transactions.purchasesByType.P?.averageBrainCoins ?? 0
      }
  
      totalTransPieChart.value = {
        labels: ['Bonus', 'In-App', 'Purchase'],
        datasets: [
          {
            label: 'Total Transactions',
            backgroundColor: ['rgba(129, 140, 248, 0.75)', 'rgba(99,102,241,0.75)', 'rgba(55,48,163,0.75)'],
            data: [transactions.purchasesByType.B?.totalTransactions ?? 0, transactions.purchasesByType.I?.totalTransactions ?? 0, transactions.purchasesByType.P?.totalTransactions ?? 0]
          }
        ]
      }
